...
Read `.claude/CLAUDE.md` when present and treat root **`AGENTS.md`** as the single source of truth. Use **`.claude/rules/`** as path-scoped hints — the files that exist are `astro.md`, `frontend.md`, `DESIGN.md`, `lead-e-pii.md`, `seo.md`, `stability.md`, `commit.md`, `deploy-vercel.md`, `mcp.md` (índice em `.claude/rules/README.md`). Load the relevant file when the review touches that area.
</role>
...
2. **Content (`astro.md` § Content Collections):** `getEntry("products", "${content.productSlug}")` for product data; copy SSOT in `${content.productJson}` (no hardcoded copy in `.astro`/`.tsx`); schema in `src/content.config.ts` moves with JSON; CTA vs navigation; canonical journey order (`${content.anchors}`).
...
5. **A11y (`frontend.md` § a11y + `DESIGN.md` § foco):** Contrast, focus, skip link, `aria-label`, alt text, heading hierarchy, reduced motion, legal link hrefs.
6. **Lead form / LGPD (`lead-e-pii.md`):** `${lead.formComponent}` has real `<label>`s, `aria-required`, accessible error/success states, LGPD consent + privacy-policy link, HTTPS; submit → `POST /api/inscricao` (planilha Google (`${lead.leadTable}`)) with WhatsApp fallback; no inline `wa.me` (use `src/lib/whatsapp.ts`, messages start `${lead.whatsappGreeting}`); endpoint/tracking secrets (`SHEETS_WEBAPP_URL`, `SHEETS_SHARED_SECRET`, `${lead.endpointEnv}`, `${tracking.pixelEnv}`, `${tracking.ga4Env}`) read from env, never committed; no PII in logs.
7. **Hooks e guardrails (`commit.md` § Protected files + `.graph-powers/config.json`):** Only describe hook behavior when reviewing `.claude/settings.json` or hook scripts — do not bypass `protect-files` or weaken bash guards.
</checks>
