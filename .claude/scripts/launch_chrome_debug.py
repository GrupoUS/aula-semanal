...
Reads start URL from .graph-powers/config.json::project.stagingUrl unless overridden via argv.
...
def get_start_url() -> str:
    ...
    # Mesma ordem de busca do loader do plugin (hooks/_config.py::CONFIG_PATHS):
    # .graph-powers/ e o legado .claude/, o primeiro que existir vence.
    config_path = next(
        (
            c
            for c in (
                Path(project_dir) / ".graph-powers" / "config.json",
                Path(project_dir) / ".claude" / "config.json",
            )
            if c.is_file()
        ),
        None,
