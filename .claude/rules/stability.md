...
> Project tooling values resolve from `.graph-powers/config.json::tooling`.
...
Project gates (route list, budget) → `.graph-powers/config.json::gates` (advisory).
...
Run after any change. Stack-specific commands resolve via `.graph-powers/config.json::tooling` placeholders.
...
- Use a package manager other than the project's declared one (`.graph-powers/config.json::tooling.packageManager`).
